...
from modulos.aplicaciones.logs import LogReport
...
class temporizador(object):
    ...
    def LogReport(self, texto, color = None):
        if self.log != None:
            self.log.EscribirLinea(Logscfg.hblTimer)
            self.log.EscribirLinea(Logscfg.hblTimer, self.name +": " +texto, color)
            if self.Status != "":
                self.log.EscribirLinea(Logscfg.hblTimer, "Estado: "+ self.Status)
